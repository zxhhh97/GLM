...
def read_context(tokenizer, args, output, text=None):
...
def generate_samples(model, tokenizer, args, device, data=None, outname = None):
    model.eval()
    output_path = "./samples"
    ...
    with torch.no_grad(), open(output_path, "a") as output:
        while True:
            ...
            if data is None:
                ...
                terminate_runs, raw_text, context_tokens_tensor, context_length = read_context(tokenizer, args, output, data)
            ...
            if mpu.get_model_parallel_rank() == 0:
                ...
                output.write("Taken time {:.2f}\nContext:{}\n".format(time.time() - start_time, raw_text))
                output.write("GLM: "+trim_decode_tokens + "\n")

            torch.distributed.barrier(group=mpu.get_model_parallel_group())
            if data:
               return trim_decode_tokens,output_path
